...
func NewConnection(client iface.Client, conn net.Conn) iface.Connection {
	c := &Connection{
		conn:       conn,
	...
	for _, handler := range client.GetChannelHandlers() {
	...
	for {
		...
		if c.decoder != nil {
			...
			for _, frame := range frames {
				...
				// 使用消息分发器分发消息，异步处理请求
				go c.client.GetDispatcher().Dispatch(req)
			}
		} else {
			// 创建一个请求体
